...
- 위 전제에 따라 권한 분리(RBAC), TLS, 로드밸런싱, 컨테이너 오케스트레이션 관련 설계는 다루지 않는다. 해당 요구가 실제로 발생하면 그 시점에 별도로 논의하고 이 문서에 반영한다.
- **인증**: Basic Auth 기반 관리자 인증을 적용한다. 상세 내용은 `auth.md`(R-014)를 참조한다.
