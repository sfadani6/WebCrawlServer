...
> 수정일: 2026-07-25
> 검토일: 2026-07-25
> 수정 이유: instructions.md 4장을 별도 문서로 분리. 4.5에 향후 확장을 대비한 예약 스텝 타입(parallel/race/delay/retry/switch/foreach) 목록 신설 — 실제 구현 전까지는 미사용 상태로 두되, 타입명을 먼저 예약해 하위 호환 문제를 예방
> 관련 문서: docs/rule/instructions.md(R-000), docs/rule/structure.md(R-003), docs/rule/scheduler.md(R-005)
> 영향 범위: 신규 분리 문서, 기존 instructions.md 4장을 대체. 4.5 예약 타입은 신규 추가이며 기존 워크플로우 동작에는 영향 없음
