...
// 최근 메시지 흐름 버퍼 (최대 100개 저장)
const messageFlowLog = [];
const MAX_FLOW_LOG = 100;
 ...
 * @param {'IN'|'OUT'} direction - 메시지 방향
 * @param {string} type - 메시지 유형 (예: ping, pong, action, log 등)
 * @param {number} size - 메시지 바이트 크기
 * @param {string|Object} summary - 메시지 요약/내용
 */
function recordMessageFlow(connectionId, direction, type = 'unknown', size = 0, summary = '') {
  const timestamp = new Date().toISOString();
  if (direction === 'IN') {
    totalInboundMessages++;
    totalInboundBytes += size;
  } else if (direction === 'OUT') {
    totalOutboundMessages++;
    totalOutboundBytes += size;
  }
  ...
  const flowItem = {
    ...
    connectionId,
    clientIp: conn ? conn.clientIp : 'unknown',
    direction,
    type,
    size,
  ...
  messageFlowLog.unshift(flowItem);
  if (messageFlowLog.length > MAX_FLOW_LOG) {
    messageFlowLog.pop();
  }

  if (conn) {
    conn.lastActivityAt = timestamp;
    if (!conn.inMessages) conn.inMessages = 0;
    if (!conn.outMessages) conn.outMessages = 0;
    if (direction === 'IN') conn.inMessages++;
    if (direction === 'OUT') conn.outMessages++;
  }
 ...
 * @returns {Array} 메시지 흐름 목록
 */
function getRecentMessageFlow(limit = 50) {
  return messageFlowLog.slice(0, limit);
}
  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (data) => {
      ...
      try {
        ...
        if (parsed.action) msgType = `${msgType}:${parsed.action}`;
      } catch (_) {
        parsed = data.toString();
      }
      recordMessageFlow(connectionId, 'IN', msgType, data.length || 0, parsed);
    });
    
    // 연결 종료 시 연결 정보 제거
    ws.on('close', () => {
      recordMessageFlow(connectionId, 'IN', 'disconnect', 0, '연결 종료됨');
      unregisterConnection(ws);
    });
    
    // 오류 시 연결 정보 제거
    ws.on('error', (err) => {
      recordMessageFlow(connectionId, 'IN', 'error', 0, err ? err.message : '웹소켓 오류');
      unregisterConnection(ws);
    });
  });
}
