...
/**
 * GET /admin/api/connections/flow
 * 실시간 WebSocket 메시지 흐름 최근 로그 조회
 */
router.get('/connections/flow', basicAuth, async (req, res, next) => {
  try {
    const limit = parseInt(req.query.limit, 10) || 50;
    const flows = getRecentMessageFlow(limit);
    return success(res, '메시지 흐름 목록 조회 성공', {
      flows,
      count: flows.length
    });
  } catch (err) {
    console.error('[admin.connections.flow] 오류:', err);
    return fail(res, '메시지 흐름 조회 실패', 500, { error: err.message });
  }
});
