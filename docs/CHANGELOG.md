...
#### 추가 (Added)
- `server/admin-ui/src/components/WebSocketDashboard.jsx`: 실시간 WebSocket 메시지 흐름 및 활성 연결 시각화 대시보드 컴포넌트 신규 생성
- `server/routes/admin.js`: GET `/admin/api/connections/flow` 엔드포인트 추가 (실시간 메시지 흐름 피드 제공)
- `docs/askLogs/ask-20260727222000.md`: 작업 처리 이력 기록

#### 변경 (Changed)
- `server/monitor/connectionManager.js`: 실시간 메시지 흐름 링 버퍼(`messageFlowLog`) 및 누적 수신/발신 바이트/건수 통계 추적 기능 구현
- `server/admin-ui/src/components/ConnectionPage.jsx`: 상단 탭 네비게이션 추가하여 WebSocket 대시보드와 클라이언트 목록을 손쉽게 전환할 수 있도록 연동
